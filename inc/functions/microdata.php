 ...
 */
if (!function_exists('event_listing_get_microdata')) :
    function event_listing_get_microdata($context)
    {
        $data = false;

        if ('microdata' !== apply_filters('event_listing_schema_type', 'microdata')) {
            return false;
        }
        ...
        if ('body' === $context) {
            ...
            $type = apply_filters('event_listing_body_itemtype', $type);
        ...
        if ('article' === $context) {
            $type = apply_filters('event_listing_article_itemtype', 'CreativeWork');
        ...
        if ($data) {
            return apply_filters("event_listing_{$context}_microdata", $data);
        }
    }
endif;
 ...
 */
if (!function_exists('event_listing_do_microdata')) :
    function event_listing_do_microdata($context)
    {
        echo event_listing_get_microdata($context); // WPCS: XSS ok, sanitization ok.
    }
endif;