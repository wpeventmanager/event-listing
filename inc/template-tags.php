            ...
            comments_popup_link(
                sprintf(
                    ...
                    /* translators: %s: post title */
                        __('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'event-listing'),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
...
if (!function_exists('event_listing_entry_footer')) :
    ...
    function event_listing_entry_footer()
    {
        ...
        if ('post' === get_post_type()) {
            ...
            /* translators: used between list items, there is a space after the comma */
            $tags_list = get_the_tag_list('', esc_html_x(', ', 'list item separator', 'event-listing'));
            if ($tags_list) {
                /* translators: 1: list of tags. */
                printf('<span class="tags-links">' . esc_html__('Tagged %1$s', 'event-listing') . '</span>', $tags_list); // WPCS: XSS OK.
            }
        }
        ...
        edit_post_link(
            sprintf(
                ...
                /* translators: %s: Name of current post. Only visible to screen readers */
                    __('Edit <span class="screen-reader-text">%s</span>', 'event-listing'),
                    array(
                        'span' => array(
                            'class' => array(),
                        ),
